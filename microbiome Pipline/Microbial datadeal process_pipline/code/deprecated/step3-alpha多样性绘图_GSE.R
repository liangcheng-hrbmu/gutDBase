...
plot_alpha_diversity_all = function(
    ...
    order_taxa = NULL,
    return_data = FALSE) {
  abundance = read.xlsx(file.path(work_dir, host, paste0(accession, "_summary.xlsx")), sheet = taxonomy, rowNames = TRUE)
  meta_data = read.xlsx(file.path(work_dir, host, paste0(accession, "_summary.xlsx")), sheet = "metaData", rowNames = TRUE)
  ...
  group_by_info = meta_data[[group_info]]
  ...
  data_plot[[group_info]] = factor(data_plot[[group_info]], levels = unique(data_plot[[group_info]]))
  ...
  # 多图合并
  alpha_agent =
    plot_alpha_diversity(data_plot, "chao1", group_info, mycolor) +
    ...
    plot_alpha_diversity(data_plot, "shannon", group_info, mycolor) +
    theme(axis.title.x = element_blank(), axis.text.x = element_text(hjust = 0.5, vjust = 0.5), plot.title = element_blank(), legend.position = "none") +
    plot_alpha_diversity(data_plot, "simpson", group_info, mycolor) +
...
taxonomy = "species"
group_info = "sex"
gg = plot_alpha_diversity_all(accession = accession, group_info = group_info, taxonomy = taxonomy, work_dir = work_dir, host = host)
gg
